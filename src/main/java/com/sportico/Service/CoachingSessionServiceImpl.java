...
public class CoachingSessionServiceImpl implements CoachingSessionService {
	...
	@Override
	public String savesession(CoachingSessiondto entity) {
		if (entity != null) {
			...
			Sports sport=sportsDao.findById(entity.getSportID()).orElseThrow();
				
			User coach=usersDao.findById(entity.getCoachID()).orElseThrow();
	...
	public List<CoachingSessiondto> getallSessionSID(Long sid) {
		Sports sport = sportsDao.findById(sid).orElse(null);
